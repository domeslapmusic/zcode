...
class user{
	...
	function validateUser($userName,$passWord,$cookie){
		...
		if(isset($userName) && $userName != "" && isset($passWord) && $passWord != ""){
			...
			$tmppass = $this->validate->userPassword($passWord);

			$q = "SELECT * FROM zb_users WHERE name='".$tmpusr."'";
			$qDB = mysql_query($q);
			$dbUser = mysql_fetch_array($qDB);

			$combinedHashed = md5($dbUser["password"].$_SESSION["key"]);

			if($tmppass == $combinedHashed){

				$this->setCurrentUser($dbUser,$cookie);
				$tmpVal = true;
