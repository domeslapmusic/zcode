...
class tv.zarate.Projects.ZBooks.zbView{
	...
	// constants
	private var BLOCK_DEPTH:Number = 2000;
	private var ADDFORM_DEPTH:Number = 3000;
	...
	*/
	...
	private function drawBookmarks():Void{
		...
		for(var x:Number=0;x<lng;x++){
			...
			if(model.getUserConfig().edit){
				...
				toDisable.push(edit_mc,delete_mc);
	...
	private function manageMainClips(action:Boolean):Void{
		...
		for(var x:String in toDisable){ toDisable[x].enabled = action; }
		
		bookmarks_mc.tabChildren = action;
		labels_mc.tabChildren = action;
		search_mc.tabChildren = action;
		buttons_mc.tabChildren = action;
		title_mc.tabChildren = action;
		pages_mc.tabChildren = action;
		login_mc.tabChildren = action;
		
		//se puede tab cuando el popup esta
